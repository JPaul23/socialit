...
const Pin = ({ pin: postedBy, image, _id, destination, save }) => {
  ...
  const alreadySaved = !!(save?.filter((item) => item.postedBy._id === user.googleId))?.length; // !! getting true or false.
  const savePin = (id) => {
    if(!alreadySaved){
            ...
            <div className='flex justify-between items-center gap-2 w-full'>
                  ...
                  {postedBy?._id === user.googleId && (
                    <button
                      ...
                      className='bg-white p-2 opacity-70 hover:opacity-100 text-white font-bold px-5 py-1 text-base rounded-3xl hover:shadow-md outlined-none'
                      >
                        <AiTwotoneDelete />
                    </button>
                  )}
